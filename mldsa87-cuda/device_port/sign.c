#include "fips202_dev.h"  /* [GPU 개조] */
#include "packing.h"
#include "params.h"
#include "poly.h"
#include "polyvec.h"
#include <string.h>  /* [GPU 개조] memcpy용 */
...
* Returns 0 (success)
**************************************************/
/* [GPU 개조] 시드를 인자로 주입(seed: SEEDBYTES 바이트). 원본은 내부 randombytes 사용. */
MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk, const uint8_t *seed) {
    ...
    /* [GPU 개조] Get randomness: 외부 주입 시드를 복사 (원본: randombytes(seedbuf, SEEDBYTES)) */
...
**************************************************/
MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(uint8_t *sig,
        size_t *siglen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
        size_t ctxlen,
        const uint8_t *sk,
        const uint8_t *rnd_in) {  /* [GPU 개조] 서명용 시드 주입(RNDBYTES). 결정론 모드면 0으로 채운 버퍼 전달 */
    unsigned int n;
    ...
    memcpy(rnd, rnd_in, RNDBYTES);  /* [GPU 개조] 원본: randombytes(rnd, RNDBYTES) */
...
* Returns 0 (success) or -1 (context string too long)
**************************************************/
MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_ctx(uint8_t *sm,
        size_t *smlen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
        size_t ctxlen,
        const uint8_t *sk,
        const uint8_t *rnd_in) {  /* [GPU 개조] 시드 주입 통과 */
    int ret;
    size_t i;
    ...
    ret = PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(sm, smlen, sm + PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES, mlen, ctx, ctxlen, sk, rnd_in);
    *smlen += mlen;
    return ret;
}
...
**************************************************/
MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify_ctx(const uint8_t *sig,
        size_t siglen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *ctx,
...
**************************************************/
MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_open_ctx(uint8_t *m,
        size_t *mlen,
        const uint8_t *sm,
        size_t smlen,
        const uint8_t *ctx,
...
MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature(uint8_t *sig,
        size_t *siglen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *sk,
        const uint8_t *rnd_in) {  /* [GPU 개조] 시드 주입 통과 */
    return PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature_ctx(sig, siglen, m, mlen, NULL, 0, sk, rnd_in);
}

MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign(uint8_t *sm,
                                      size_t *smlen,
                                      const uint8_t *m,
                                      size_t mlen,
                                      const uint8_t *sk,
                                      const uint8_t *rnd_in) {  /* [GPU 개조] 시드 주입 통과 */
    return PQCLEAN_MLDSA87_CLEAN_crypto_sign_ctx(sm, smlen, m, mlen, NULL, 0, sk, rnd_in);
}

MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify(const uint8_t *sig,
        size_t siglen,
        const uint8_t *m,
        size_t mlen,
        const uint8_t *pk) {
...
MLD_HD int PQCLEAN_MLDSA87_CLEAN_crypto_sign_open(uint8_t *m,
        size_t *mlen,
