 ...
 **************************************************/
__device__ static void keccak_absorb(uint64_t *s, uint32_t r, const uint8_t *m,
 ...
 *              - uint32_t r: rate in bytes (e.g., 168 for SHAKE128)
 **************************************************/
__device__ static void keccak_squeezeblocks(uint8_t *h, size_t nblocks,
 ...
 **************************************************/
__device__ static void keccak_inc_init(uint64_t *s_inc) {
    size_t i;
 ...
 *              - size_t mlen: length of input in bytes
 **************************************************/
__device__ static void keccak_inc_absorb(uint64_t *s_inc, uint32_t r, const uint8_t *m,
                              size_t mlen) {
    size_t i;
 ...
 *                                 Keccak-derived functions
 **************************************************/
__device__ static void keccak_inc_finalize(uint64_t *s_inc, uint32_t r, uint8_t p) {
 ...
 *              - uint32_t r: rate in bytes (e.g., 168 for SHAKE128)
 **************************************************/
__device__ static void keccak_inc_squeeze(uint8_t *h, size_t outlen,
...
__device__ void shake128_inc_ctx_clone(shake128incctx *dest, const shake128incctx *src) {
    memcpy(dest->ctx, src->ctx, PQC_SHAKEINCCTX_BYTES);
}

__device__ void shake128_inc_ctx_release(shake128incctx *state) {
}
...
__device__ void shake256_inc_ctx_release(shake256incctx *state) {
}
...
__device__ void shake128_ctx_clone(shake128ctx *dest, const shake128ctx *src) {
...
__device__ void shake256_ctx_clone(shake256ctx *dest, const shake256ctx *src) {
    memcpy(dest->ctx, src->ctx, PQC_SHAKECTX_BYTES);
}
 ...
 *              - size_t inlen: length of input in bytes
 **************************************************/
__device__ void shake128(uint8_t *output, size_t outlen,
 ...
 *              - size_t inlen: length of input in bytes
 **************************************************/
__device__ void shake256(uint8_t *output, size_t outlen,
              const uint8_t *input, size_t inlen) {
...
__device__ void sha3_256_inc_init(sha3_256incctx *state) {
    keccak_inc_init(state->ctx);
}

__device__ void sha3_256_inc_ctx_clone(sha3_256incctx *dest, const sha3_256incctx *src) {
    memcpy(dest->ctx, src->ctx, PQC_SHAKEINCCTX_BYTES);
}

__device__ void sha3_256_inc_ctx_release(sha3_256incctx *state) {
}

__device__ void sha3_256_inc_absorb(sha3_256incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, SHA3_256_RATE, input, inlen);
}

__device__ void sha3_256_inc_finalize(uint8_t *output, sha3_256incctx *state) {
...
__device__ void sha3_384_inc_init(sha3_384incctx *state) {
    keccak_inc_init(state->ctx);
}

__device__ void sha3_384_inc_ctx_clone(sha3_384incctx *dest, const sha3_384incctx *src) {
    memcpy(dest->ctx, src->ctx, PQC_SHAKEINCCTX_BYTES);
}

__device__ void sha3_384_inc_absorb(sha3_384incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, SHA3_384_RATE, input, inlen);
}

__device__ void sha3_384_inc_ctx_release(sha3_384incctx *state) {
}

__device__ void sha3_384_inc_finalize(uint8_t *output, sha3_384incctx *state) {
 ...
 *              - size_t inlen:   length of input in bytes
 **************************************************/
__device__ void sha3_384(uint8_t *output, const uint8_t *input, size_t inlen) {
    uint64_t s[25];
    uint8_t t[SHA3_384_RATE];
...
__device__ void sha3_512_inc_init(sha3_512incctx *state) {
    keccak_inc_init(state->ctx);
}

__device__ void sha3_512_inc_ctx_clone(sha3_512incctx *dest, const sha3_512incctx *src) {
    memcpy(dest->ctx, src->ctx, PQC_SHAKEINCCTX_BYTES);
}

__device__ void sha3_512_inc_absorb(sha3_512incctx *state, const uint8_t *input, size_t inlen) {
    keccak_inc_absorb(state->ctx, SHA3_512_RATE, input, inlen);
}

__device__ void sha3_512_inc_ctx_release(sha3_512incctx *state) {
}

__device__ void sha3_512_inc_finalize(uint8_t *output, sha3_512incctx *state) {
